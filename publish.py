...
for rid in rid_list:
  result = subprocess.run(f"dotnet publish BlossomiShymae.SkipNSlash.Desktop/BlossomiShymae.SkipNSlash.Desktop.csproj -c Release -p:PublishSingleFile=true -o dist -r {rid} --no-self-contained", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
